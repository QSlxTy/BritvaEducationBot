...
from keyboards.admin.admin_keyboard import choose_delete_kb
...
async def choose_delete_user(call: types.CallbackQuery, state: FSMContext, session_maker: sessionmaker):
    ...
    try:
        msg = await data['msg'].edit_text(
            text=f'<b>Вы точно хотите закрыть доступ к курсу пользователю...\n\n'
...
async def delete_user(call: types.CallbackQuery, state: FSMContext, session_maker: sessionmaker):
    ...
    if 'yes' in call.data:
        ...
        try:
            msg = await data['msg'].edit_text(
                text=f'<b>Вы отменили процесс закрытия курса ⭕️ /start</b>'
            )
        except (TelegramBadRequest, KeyError) as _ex:
            logging.error(_ex)
            await call.message.delete()
